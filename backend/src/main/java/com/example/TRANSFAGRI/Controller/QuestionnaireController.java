...
public class QuestionnaireController {
    ...
    public ResponseEntity<Questionnaire> generateQuestionnaire(@RequestBody QuestionnaireRequestDto dto) {
        ...
        try {
            // Sérialiser chaque question en JSON pour stockage
            List<String> questionsJson = questions.stream()
                .map(q -> {
                    try { return objectMapper.writeValueAsString(q); } catch (Exception e) { return null; }
                })
                .filter(q -> q != null)
                .toList();
